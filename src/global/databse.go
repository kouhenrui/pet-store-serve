...
// 初始化链接
func Dbinit() {
	dsn := fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=%s&parseTime=True&loc=Local", mysqlConf.UserName, mysqlConf.PassWord, mysqlConf.HOST, mysqlConf.DATABASE, mysqlConf.CHARSET)
	//连接MYSQL, 获得DB类型实例，用于后面的数据库读写操作。
	MysqlDClient, err = gorm.Open(mysql.Open(dsn), &gorm.Config{
